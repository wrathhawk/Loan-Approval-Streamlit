...
cb_person_default_on_file = st.selectbox("Default on File (did the customer failed to repay a loan in the past?)", ["Y", "N"])

income = st.number_input("Annual Income", min_value=0)
loan_amount = st.number_input("Loan Amount", min_value=0)
person_emp_length = st.number_input("Person Employment Length", min_value=0)
loan_percent_income = st.number_input("Loan Percent Income (in 0.0 format)", min_value=0.01)
